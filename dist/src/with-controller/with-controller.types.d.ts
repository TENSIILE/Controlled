...
/**@public */
export type ControlledComponentsType<T extends string = string> = {
    [key in T]: FC<never>;
};
/**@internal */
...
export interface ControlledGroup<InputFields extends FieldValues> {
    asyncDefaultValue?: PathValue<InputFields, Path<InputFields>> | Promise<PathValue<InputFields, Path<InputFields>>> | ((inputFields: InputFields) => PathValue<InputFields, Path<InputFields>>) | null;
...
/**@internal */
export type ReactHookFormProps<InputFields extends FieldValues> = Pick<NativeComponentProps<InputFields, AnonimFunction>, 'name' | 'defaultValue' | 'rules' | 'onAction' | 'controlledGroup'>;
/**@internal */
...
export interface ControllerConfig<Props, OnChangePropName extends keyof Props | 'onChange' | null> {
    adapter?: (value: string, onChange: OnChangePropName extends null ? AnonimFunction : (Props & {
        onChange: unknown;
    })[OnChangePropName & string], options: {
        props: Props;
